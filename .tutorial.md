...
Depending on the platform setup, the asset may initialize automatically or may need to be manually executed. To manually execute the asset, run the command `./gcp-istio-scp.sh` on the terminal.
...
## Explore Istio Single Control Plane Network Peering

This asset enables you to explore Istio single control plane network peering on the Google Cloud. 
...
## Step 4: Configure network peering

Enter `4` at the `additional steps to perform` prompt to create Kubernetes cluster.
...
Enter `5` at the `additional steps to perform` prompt to create a Kubernetes cluster. 
...
Enter `6` at the `additional steps to perform` prompt to create firewall rule.

Note the comments associated with each command.

To create firewall rule, click the **NEXT** button.

## Step 7: Install Istio

Enter `7` at the `additional steps to perform` prompt to install Istio services mesh.
...
## Step 8: Configure service and deployment

Enter `8` at the `additional steps to perform` prompt to configure service and deployment.
